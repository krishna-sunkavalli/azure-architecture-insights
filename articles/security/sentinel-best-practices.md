...
|---|---|
| [Data Collection and Connectors](#data-collection-and-connectors) | Connector sequencing, DCR filtering, Logstash penalties, workspace scope, CEF/Syslog forwarder sizing, connector builder agent |
| [Data Tiers, Retention, and Cost](#data-tiers-retention-and-cost) | Analytics vs lake tier placement, retention windows, federation, summarization, commitment tiers, cost estimator |
| [Detection Engineering](#detection-engineering) | Rule hygiene, ASIM normalization, NRT rules, use case mapping, DCR transformations, UI-based filtering |
| [Identity, UEBA, and Enrichment](#identity-ueba-and-enrichment) | UEBA enablement, watchlist design |
| [KQL and Threat Hunting](#kql-and-threat-hunting) | Query optimization, identity graph traversal, custom graphs (GQL), notebook use case boundary, MSTICPy, AML compute sizing |
| [SOC Operations](#soc-operations) | Incident workflow, RBAC, row-level RBAC, multi-workspace decisions, SIEM migration, Defender portal migration |
| [Content and Threat Intelligence](#content-and-threat-intelligence) | CI/CD pipelines, Content Hub lifecycle, TI activation |
| [Automation](#automation) | Automation rules vs playbooks, managed identity, Logic App tiers, Function Apps, testing, playbook generator, data lake KQL API |
| [Log Routing to the Data Lake Tier](#when-to-route-logs-to-the-data-lake-tier) | Per-source routing decisions |
| [KQL Validation Queries](#kql-queries-to-validate-your-environment) | 16 queries to validate your environment |
| [Security Copilot Cross-Dependencies](#security-copilot-cross-dependencies-and-what-breaks-without-them) | Sentinel configurations that directly affect Copilot output quality, MCP server and agentic patterns |
...
| **Logstash filtering trade-off** | Avoid using Logstash to filter content from logs that are on the free-tier list (Windows Security Events, Syslog via connector). Logstash re-ingests filtered output as custom logs, converting free-tier data to paid and removing UEBA, ML, and fusion support. The rebuilt Java-based output plugin (`microsoft-sentinel-log-analytics-logstash-output-plugin 2.0.0-java`) replaces the legacy Ruby plugin and uses the Azure Monitor Logs Ingestion API with DCRs instead of the deprecated HTTP Data Collector API. It supports managed identity authentication, eliminating stored credentials. Use the new plugin for all new Logstash deployments. | The cost and capability penalties from free-tier conversion remain regardless of plugin version: Logstash output lands in custom tables. The new plugin does not change the table routing behavior, only the ingestion mechanism and credential model. |
...
| **Custom connector development** | Use the Sentinel connector builder agent in VS Code (GitHub Copilot Chat) to accelerate custom connector development. Provide the target API's documentation URL and the agent generates the schema, DCR, deployment assets, connector UI, secret handling, and polling logic. Validate the generated connector against the API before ingestion. This is the fastest path for teams building connectors for sources not covered by the Content Hub. | The agent generates an initial connector scaffold, not a production-ready connector. Connectors for APIs with complex authentication flows or non-standard pagination require manual refinement. Review generated schemas against the actual API response structure before deploying. |

### Data Tiers, Retention, and Cost
...
| **Commitment tier pricing** | Switch from pay-as-you-go to a Commitment Tier at 100 GB/day ingestion. The discount is meaningful at that volume and increases with scale. Organizations above 500 GB/day should evaluate the dedicated cluster option for further savings and improved query performance. Use the Sentinel Cost Estimator (available at microsoft.com/security/pricing/microsoft-sentinel/cost-estimator) to model meter-level cost breakdowns, 3-year projections, ingestion tier splits, and retention requirements before committing to a tier. E3 to E5 upgrades may qualify for free ingestion credits against user count; run the estimator before assuming the commitment tier is the only lever. | Commitment tiers require predicting ingestion volume. Over-committing wastes budget. Under-committing negates the discount. Sample log sources for a full week before selecting a tier. |
...
| **DCR ingestion-time transformations** | Use KQL transformation rules in Data Collection Rules to enrich, filter, or reshape data at ingestion time before it reaches the Log Analytics table. This is more powerful than agent-side filtering: transformations can parse fields, add calculated columns, drop rows conditionally, and route data to different tables from a single source. Filtering and splitting are now configurable directly in the Defender portal UI using KQL, without writing custom JSON configuration files. Use the portal-based workflow for new configurations; reserve JSON-based DCR authoring for complex transformations the UI does not yet expose. | DCR transformations are applied before storage, so dropped rows cannot be recovered. Transformations that contain logic errors silently discard data. Test all transformation KQL in a staging DCR before applying to production ingestion. |

### Identity, UEBA, and Enrichment
...
| **Custom graph traversal** | Build identity relationship graphs (group membership, SP permissions) via Sentinel VS Code extension notebooks. Schedule as recurring jobs. For more complex graph-based correlation, use custom graphs built with Graph Query Language (GQL) in VS Code, powered by Microsoft Fabric. Custom graphs allow you to build tailored security graphs from data lake data and non-Microsoft sources, surface hidden attack paths, and visualize relationships between identities, devices, and alerts. Custom graph queries are billed via the Sentinel graph meter. | KQL-based identity graph traversal has 30-day default retention on on-demand schedules. GQL custom graphs are a distinct capability from KQL graph traversal and require Fabric integration. The GQL authoring workflow and the VS Code extension are separate tools. Use KQL graph traversal for ad-hoc identity pivoting; use custom graphs when the relationship model needs to be persistent, reusable, and shared across the team. |
| **Notebook use case boundary** | Use Sentinel notebooks (Jupyter via Azure Machine Learning) for workflows that exceed what KQL analytics rules and workbooks can handle: long-running queries over months of data, ML-based anomaly scoring, multi-step investigation chains that require intermediate state, and Python-based enrichment from external APIs. Do not replicate detection rule logic in notebooks. If a query can finish in a KQL workbook in under 30 seconds, it does not belong in a notebook. | Notebooks require an Azure ML workspace attached to the Sentinel workspace. Compute costs accrue while the kernel is active. Analysts without Python skills cannot execute or modify notebook-based investigations without support. |
| **MSTICPy as the standard library** | Use MSTICPy (Microsoft Threat Intelligence Python Security Tools) as the base library for all Sentinel notebook development. MSTICPy provides pre-built connectors to the Sentinel workspace, MDTI enrichment, entity pivots, and timeline visualizations. It is the library Microsoft builds its own Sentinel investigation notebooks against. | MSTICPy requires Python 3.8+. Its Azure ML integration assumes the notebook is running in an attached AML compute instance, not a generic Jupyter server. Local development against MSTICPy requires explicit workspace credential configuration. |
| **Notebook compute sizing and versioning** | For interactive investigation, use a Standard_DS3_v2 (4 vCPU, 14 GB) compute instance as the baseline. For scheduled notebook jobs (recurring threat hunting, weekly entity summarization), use AML compute clusters with autoscale-to-zero to eliminate idle cost. Treat all investigation notebooks as code: store them in Git via the Sentinel GitHub integration and enforce peer review before promoting notebooks used in recurring jobs to production. | Compute instances that are not stopped between sessions accumulate cost continuously. Without Git versioning, investigation notebooks drifting from their original tested state is common and hard to detect. |
...
| **RBAC role assignment** | Assign the minimum Sentinel role per function: Reader for analysts querying data, Responder for SOC tier-1 closing incidents, Contributor for engineers building rules and connectors. Apply table-level RBAC for sensitive data types that specific teams should not see. For environments where multiple SOC teams share a single workspace, use row-level RBAC to scope what each team can see within a table. Row-level RBAC applies reusable scope definitions uniformly across tables without workspace splitting, and is the right model for MSSPs and enterprises with distinct business unit security teams. | Table-level RBAC is applied per Log Analytics table, not per Sentinel workspace. Row-level RBAC is a distinct mechanism from table-level RBAC and requires separate configuration. Both add ongoing access management overhead. |
...
| **SIEM migration from Splunk or QRadar** | Use the in-product SIEM migration experience to translate existing Splunk and QRadar detection logic to Sentinel analytics rules. Upload your Splunk SPL or QRadar AQL exports and the tool generates recommended Sentinel rules with connector mapping and phased migration tracking. Start with your highest-fidelity detection rules and validate output before decommissioning the source SIEM. | The migration tool covers common detection patterns well. Rules that rely on proprietary lookups, advanced macros, or source-SIEM-specific enrichment functions require manual rewrite. The tool reduces migration time but does not eliminate the need for detection validation in the target environment. |
...
| **Playbook generator** | Use the playbook generator in Security Copilot to build new playbooks from natural language descriptions. The generator produces Python playbooks with documentation, a visual flowchart, and built-in tests. It integrates with third-party APIs through integration profiles that define the base URL, authentication method, and credentials without requiring predefined connectors. Validate generated playbooks against real alerts before enabling in production. Prerequisites: a Security Copilot workspace (EU or US capacity), Sentinel onboarded to the Defender portal, and Sentinel Contributor role. | The generator outputs Python playbooks, not Logic Apps. Python playbooks use a different execution model from Logic App-based playbooks and require Python-capable infrastructure to run. Teams already invested in Logic Apps workflows cannot reuse the generated output directly. |
| **Programmatic data lake access** | Use the data lake KQL API to query Sentinel data lake tables from automation workflows, AI agents, and CI/CD pipelines. The endpoint is `POST https://api.securityplatform.microsoft.com/lake/kql/v2/rest/query` with scope `4500ebfb-89b6-4b14-a480-7f749797bfcd/.default`. Service principals authenticating against this API require Azure RBAC roles on the workspace, not Entra ID roles or XDR unified RBAC roles. Query body format: `{"csl": "<KQL>", "db": "<workspacename-workspaceid>"}`. Queries must be single-line JSON strings. | The API is subject to result size and execution time limits. Complex historical queries that timeout interactively should be restructured as notebook jobs or scheduled analytics. The API is appropriate for lightweight enrichment calls and event-driven automation, not for bulk historical extraction. |
...
Organizations with Microsoft 365 E5 licenses now have Security Copilot included. Microsoft began auto-provisioning for eligible tenants in early 2026 with a 30-day advance notification before activation. Customers without E5 provision access by purchasing Security Compute Units (SCUs) through the Azure portal. Either way, how you configure Sentinel directly determines how much value Security Copilot can deliver, and most teams only discover the dependency gaps after the product is live in their tenant.
...
| **Sentinel MCP server** | The Sentinel MCP server exposes Sentinel data lake capabilities to AI agents and MCP-compatible clients as a programmatic access layer. Use it to build agentic SOC workflows where an AI agent can query tables, look up entity risk, investigate incidents, and correlate signals through natural language interactions. The entity analyzer (GA) provides URL and user identity risk assessments billed through SCU consumption. The MCP graph tool collection enables visualization of relationships between identities, devices, and alerts. MCP connectivity is not limited to Microsoft tools: a custom MCP connector for Anthropic Claude is available for organizations using non-Microsoft AI. | MCP-based access is billed separately from Sentinel ingestion. Entity analyzer queries consume SCUs. Agents that issue high-frequency queries against the data lake without rate limiting will accumulate SCU costs proportional to query volume. Design agentic workflows with explicit query budgets and circuit breakers. |
